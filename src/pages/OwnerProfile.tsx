...
import { Upload, CheckCircle, Clock, Building2 } from "lucide-react";
import { toast } from "sonner";
import Navbar from "@/components/Navbar";

const OwnerProfile = () => {
  const [name] = useState("Aishik Pramanik");
  const [email] = useState("aishik.owner@example.com");
  const [phone, setPhone] = useState("");
  const [aadhaarFront, setAadhaarFront] = useState<File | null>(null);
  const [aadhaarBack, setAadhaarBack] = useState<File | null>(null);
  const [propertyProof, setPropertyProof] = useState<File | null>(null);
  ...
  const handlePropertyProofUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      setPropertyProof(e.target.files[0]);
      toast.success("Property proof uploaded");
    }
  };

  const handleSubmitVerification = () => {
    if (!aadhaarFront || !aadhaarBack || !propertyProof) {
    ...
    setTimeout(() => {
      ...
      console.log("STEP 6 COMPLETE: Owner signup and profile with Aadhaar verification added");
    }, 1500);
  };
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="mb-6">
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                <h3 className="font-semibold mb-4">Step 2: Property Ownership Proof</h3>
                <div className="space-y-2">
                  ...
                  <div className="border-2 border-dashed rounded-lg p-6 text-center hover:border-primary transition-colors cursor-pointer">
                    <input
                      type="file"
                      accept="image/*,.pdf"
                      onChange={handlePropertyProofUpload}
                      className="hidden"
                      id="property-proof"
                    />
                    <label htmlFor="property-proof" className="cursor-pointer">
                      <Upload className="h-8 w-8 mx-auto mb-2 text-muted-foreground" />
                      {propertyProof ? (
                        <p className="text-sm text-green-600">{propertyProof.name}</p>
                      ) : (
                        <p className="text-sm text-muted-foreground">Click to upload (Property deed, tax receipt, etc.)</p>
                      )}
                    </label>
                  </div>
                </div>
              ...
              {!verified && (
                <Button 
                  ...
                  className="w-full"
                  disabled={!aadhaarFront || !aadhaarBack || !propertyProof}
                >
